...
let startIdx = lines.findIndex(l => l.includes('function estimateDimensionsFromWeight('));
if (startIdx !== -1) {
    let endIdx = startIdx;
    while (!lines[endIdx].startsWith('}')) { endIdx++; }
    lines.splice(startIdx - 6, endIdx - startIdx + 7, newFunc);
    content = lines.join('\n');
}
